...
void Canny::detect(const char* filename, int core_size, int threshold_low, int threshold_high) {
    img = CImg<byte>(filename);
    vector<vector<double>> filter = create_filter(core_size, core_size, 1);

    // 将图片转为灰度图
    grayscaled = luminance(img);
    ...
    // 去除小于 20 长度的边缘
    CImg<byte> final_result = link_contours(thres_result);
    final_result.save_bmp("6.final_result.bmp");
}

CImg<byte> Canny::luminance(CImg<byte> img) {
    CImg<byte> newImg = CImg<byte>(img.width(), img.height());
...
CImg<byte> Canny::gaussian(CImg<byte> imgIn, vector<vector<double>> filter)
{
    int size = (int)filter.size() / 2;
    CImg<byte> filteredImg = CImg<byte>(imgIn.width() - 2 * size, imgIn.height() - 2 * size, imgIn.depth());
...
CImg<byte> Canny::sobel()
{
    ...
    CImg<byte> filteredImg = CImg<byte>(gradient.width() - 2 * size, gradient.height() - 2 * size);
...
CImg<byte> Canny::non_max_suppression()
{
    CImg<byte> nonMaxSupped = CImg<byte>(sobel_filtered.width() - 2, sobel_filtered.height() - 2);
