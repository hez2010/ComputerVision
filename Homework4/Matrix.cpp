...
Matrix Matrix::create_identity(int size)
{
...
Matrix Matrix::band_solve(Matrix A, Matrix b, int k)
{
...
double Matrix::dot_product(Matrix a, Matrix b)
{
    double sum = 0;
    for (int i = 0; i < a.rows_; ++i) {
        sum += (a(i, 0) * b(i, 0));
...
Matrix Matrix::gaussian_eliminate()
{
    Matrix Ab(*this);
    int rows = Ab.rows_;
    int cols = Ab.cols_;
...
Matrix Matrix::row_reduce_from_gaussian()
{
    Matrix R(*this);
    int rows = R.rows_;
    int cols = R.cols_;
...
void Matrix::read_solutions_from_rref(ostream& os)
{
    Matrix R(*this);
...
{
    Matrix I = Matrix::create_identity(rows_);
    Matrix AI = Matrix::augment(*this, I);
    Matrix U = AI.gaussian_eliminate();
    Matrix IAInverse = U.row_reduce_from_gaussian();
...
Matrix Matrix::exp_helper(const Matrix& m, int num)
{
    if (num == 0) { 
        ...
        return exp_helper(m * m, num/2);
    } else {                    // num is odd
        return m * exp_helper(m * m, (num-1)/2);
    }
}
